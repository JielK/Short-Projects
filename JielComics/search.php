...
if(isset($_POST['search'])){
    $stmt=$conn->prepare("SELECT * FROM comics where title like '%".$search."%' ORDER BY comic_id DESC LIMIT 40");
    $stmt->execute();
...
<div class="latest">
  <span class="latest-text mx-4">Search Results for "<?php echo $search; ?>"</span>
    <?php 
      if(isset($_GET['admin'])){
              echo '<a href="add_comic.php" ><button class="adds"><i class="fa-solid fa-plus"></i> &nbspADD</button></a>';    
      }
    ?>
  <hr class="chunkyhr">
  <div class="mt-2 row mx-auto container-fluid">
    <?php while($row = $searchresults->fetch_assoc()) { ?>
      <div onclick="window.location.href='<?php echo "comic.php?comic_id=". $row['comic_id']?>';" class="home-comics col-lg-3 col-md-4 col-sm-12 pt-2">
        <div class="container">
        ...
        <a class="chapter mx-2" href="index.php"><i class="fas fa-circle text-primary"></i>&nbsp&nbspChapter <?php echo $rowc['chapter_number']; ?></a><br>
        <?php } ?>
        <?php
        if(isset($_GET['admin'])){
